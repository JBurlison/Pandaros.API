...
namespace Pandaros.API.AI
{
    public static class NameGenerator
    {
        ...
        static NameGenerator()
        {
            ...
            var fnames = File.ReadAllText(GameInitializer.MOD_FOLDER + "/AI/FirstNames.csv").Replace("\r", "");
            fnames.Replace(" ", "");
            var afirstNames = fnames.Split('\n');
            _firstNames = afirstNames.OfType<string>().ToList();

            var lnames = File.ReadAllText(GameInitializer.MOD_FOLDER + "/AI/LastNames.csv").Replace("\r", "");
            lnames.Replace(" ", "");
